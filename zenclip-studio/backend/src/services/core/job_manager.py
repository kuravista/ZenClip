...
class JobManager:
    ...
    def __init__(self):
        ...
        # In-memory specific properties
        self.job_queue = PriorityQueue()  # Priority queue: lower number = higher priority
    ...
    def _worker_loop(self):
        while self.running:
            try:
                # Wait for job from priority queue
                queue_item = self.job_queue.get()
                
                # Handle stop signal
                if queue_item is None:
                    break
                
                # Extract job_id from priority tuple: (priority, timestamp, job_id)
                if isinstance(queue_item, tuple):
                    _, _, job_id = queue_item
                else:
                    # Fallback for compatibility (shouldn't happen with new code)
                    job_id = queue_item
                ...
                try:
                    # Import runner locally to avoid circular imports
                    from services.core.job_runner import JobRunner
                    runner = JobRunner(job_id, self.jobs_dir)
                    runner.run()
                except Exception as e:
    ...
    def submit_job(self, job_data, priority=5):
        """
        ...
        initial_state = {
            ...
            "priority": priority,  # Store priority in metadata
            "data": job_data,
            "history": [] # Track state changes
        }
        
        
        self._save_job_file(job_id, "metadata.json", initial_state)
        
        
        # Add to Priority Queue
        # Format: (priority, timestamp, job_id)
        # timestamp ensures FIFO for jobs with same priority
    ...
    def get_job(self, job_id):
        return self._load_job_file(job_id, "metadata.json")
    ...
    def update_job_status(self, job_id, status: JobStatus, progress_msg=None, error_msg=None, progress_percent=None):
        ...
        self._save_job_file(job_id, "metadata.json", meta)

        # Disk update is atomic via os.replace in _save_job_file
        # No in-memory cache 'self.jobs' exists currently, so get_job reads from disk.
    ...
    def update_job_data(self, job_id, updates):
        ...
        # Deep update or shallow? Shallow merge of 'data' dict.
        if 'data' not in meta: meta['data'] = {}
    ...
    def resume_job(self, job_id):
        ...
        if meta and meta['status'] not in ['queued', 'processing', 'running']: # Wait, checking internal statuses
             ...
             # Get original priority or use default
