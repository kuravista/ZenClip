...
def run_ffmpeg_clip(
    ...
    if watermark_image:
        ...
        wm_config = watermark_config or {}
        wm_opacity = wm_config.get('opacity', 0.3)
        # watermark_size: scale factor (0.0-1.0) → fontsize relative to video WIDTH
        # so the watermark text fits within the video frame.
        # At size=0.3 → fontsize ≈ 30px (readable but not dominant)
        wm_size = wm_config.get('fontsize', 24)
        if wm_size <= 1:
            # Treat as scale factor: 0.3 = ~30px, 0.5 = ~50px, etc.
            wm_fontsize = max(14, int(target_w * wm_size * 0.1))
        else:
