...
if getattr(sys, 'frozen', False):
    ...
    if os.path.exists(ffmpeg_path):
        ...
        print(f"[ERROR] {ffmpeg_name} not found at: {ffmpeg_path} - App may likely fail if assuming strict bundling.")
            ...
            try:
                ...
                for conn in proc.connections(kind='inet'):
                    if conn.laddr.port == port:
                        print(f"[BUSY] Port {port} is busy. Killing process {proc.pid} ({proc.name()})...")
                        proc.kill()
                        proc.wait(timeout=3)
