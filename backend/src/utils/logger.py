...
# ── File logging ──────────────────────────────────────────────────────────────
_LOG_FILE = os.path.join(tempfile.gettempdir(), "zenclip_backend.log")

def _write_to_log_file(raw_line: str):
    """Append a log line to the temp log file (no colors)."""
    try:
        with open(_LOG_FILE, "a", encoding="utf-8") as f:
            # Strip ANSI color codes for file output
            import re
            clean = re.sub(r'\033\[[0-9;]*m', '', raw_line)
