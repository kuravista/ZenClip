...
def realign_words(phrase):
    ...
    if len(new_split) == len(old_split):
        return [
            {"text": new_split[i], "start": words[i]["start"], "end": words[i]["end"]}
    ...
    for i, w in enumerate(new_split):
        ...
        if real_end > end_time:
            real_end = end_time

        new_words_timings.append({"text": w, "start": current_time, "end": real_end})
...
def generate_rapid_fire_events(timings, clip_start, clip_end, style_config):
    ...
    for i in range(len(all_words)):
        ...
        events.append({"start": start, "end": real_end, "text": final_text})
...
def generate_karaoke_events(timings, clip_start, clip_end, style_config):
    ...
    for phrase in timings:
        ...
        for w in words:
            ...
            if w_end < -1.5 or w_start > (clip_end - clip_start + 1.5):
                continue
            all_words.append({"text": w["text"], "start": w_start, "end": w_end})
...
def generate_mozi_events(timings, clip_start, clip_end, style_config):
    """
    Mozi style — per-word green highlight, single line, stable position.

    Like rapid_pro approach:
      - One event per chunk with fixed \pos(x,y) — no jumping.
      - Chunk size = max 3 words (fits one line).
      - Per-word highlight: active word = bright green + 1.5x,
        other words = dim gray at normal size.
      - One event PER WORD (same \pos) so highlight animates left→right.
    """
    events = []
    ...
    # 2. Chunking — smart: max 5 words OR max 22 chars, whichever hits first
    ...
    for w in all_words:
        wlen = len(w["text"])
        needed = wlen + (1 if current_chunk else 0)  # +1 for space
        if (
            current_chars + needed > MAX_CHARS or len(current_chunk) >= MAX_WORDS
        ) and current_chunk:
            chunks.append(current_chunk)
            current_chunk = []
            current_chars = 0
    ...
    # Fixed position (same as rapid_pro)
    ...
    # 4. Generate events — one per word per chunk
    for chunk in chunks:
        if not chunk:
            continue
        ...
        for i, word in enumerate(chunk):
            ...
            # Build text: all words in chunk visible,
            # active word gets green + 1.5x, rest dim gray
            parts = []
            for j, w in enumerate(chunk):
                w_text = sanitize_ass_text(w["text"])
                if j == i:
                    parts.append(
                        f"{{\\c{active_color}\\fs{active_fontsize}\\b1}}{w_text}"
                        f"{{\\c{dim_color}\\fs{fontsize}\\b0}}"
                    )
                else:
            ...
            # Fade in on first word of chunk
            fade_tag = "\\fad(120,80)" if i == 0 else ""

            events.append(
                {
                    "start": w_start,
                    "end": w_end,
                    "text": f"{{{pos_tag}{fade_tag}}}{full_text}",
                    "Style": "Default",
                }
            )

        # Brief fade-out showing all dim
        if duration > 0.5:
            parts = []
            for w in chunk:
                w_text = sanitize_ass_text(w["text"])
            ...
            events.append(
                {
                    "start": chunk_end,
                    "end": chunk_end + 0.15,
                    "text": f"{{{pos_tag}\\fad(0,100)}}{' '.join(parts)}",
                    "Style": "Default",
                }
            )
...
def generate_rapid_pro_events(timings, clip_start, clip_end, style_config):
    ...
    for chunk in chunks:
        ...
        events.append(
            {"start": chunk_start, "end": chunk_end, "text": glow_text, "Layer": 0}
        )

        # Layer 1: Sharp text face
        front_text = f"{{\\b1\\bord1\\blur0{fade_tag}{pos_tag}}}{front_text_content}"
        events.append(
            {"start": chunk_start, "end": chunk_end, "text": front_text, "Layer": 1}
        )
...
def generate_pod_d_events(timings, clip_start, clip_end, style_config):
    ...
    for chunk in chunks:
        ...
        events.append(
            {"start": chunk_start, "end": chunk_end, "text": final_text, "Layer": 1}
        )
    ...
    for chunk in chunks:
        ...
        for i, w in enumerate(chunk):
            ...
            if is_highlight:
                ...
                part = (
                    f"{{\\fn{font_highlight}\\i1\\b1\\c{highlight_ass}\\bord2}}{w_text}"
                )
            else:
        ...
        events.append({"start": chunk_start, "end": chunk_end, "text": final_text})
...
def generate_phrase_events(timings, clip_start, clip_end, style_config):
    ...
    for phrase in timings:
        ...
        events.append({"start": p_start, "end": p_end, "text": f"{fade_tag}{text}"})
...
def generate_prince_events(timings, clip_start, clip_end, style_config):
    ...
    for chunk in chunks:
        ...
        events.append({"start": chunk_start, "end": chunk_end, "text": full_text})
...
def generate_boxies_events(timings, clip_start, clip_end, style_config):
    ...
    for chunk in chunks:
        ...
        line1_text = " ".join(sanitize_ass_text(w["text"].upper()) for w in line1_words)

        # Line 2: Normal text with colored background box
        line2_text = " ".join(sanitize_ass_text(w["text"]) for w in line2_words)
        ...
        if line2_text:
            ...
            line2_styled = (
                f"{{\\c{white_ass}{font_tag}\\bord12\\3c{bg_ass}\\blur0}}{line2_text}"
            )
...
def generate_ass_file(
    filepath, phrase_timings, clip_start, clip_end, video_size, style_config=None
):
    """
    ...
    if style == "karaoke":
        events = generate_karaoke_events(
            phrase_timings, clip_start, clip_end, style_config
        )
    elif style == "mozi":
        events = generate_mozi_events(
            phrase_timings, clip_start, clip_end, style_config
        )
    elif style == "rapid":
        events = generate_rapid_fire_events(
            phrase_timings, clip_start, clip_end, style_config
        )
    elif style == "rapid_pro":
        events = generate_rapid_pro_events(
            phrase_timings, clip_start, clip_end, style_config
        )
    elif style == "pod_d":
        events = generate_pod_d_events(
            phrase_timings, clip_start, clip_end, style_config
        )
    elif style == "elegant":
        events = generate_elegant_events(
            phrase_timings, clip_start, clip_end, style_config
        )
    elif style == "shadow":
        ...
        events = generate_boxies_events(
            phrase_timings, clip_start, clip_end, style_config
        )
    elif style == "prince":
        events = generate_prince_events(
            phrase_timings, clip_start, clip_end, style_config
        )
    else:
        # Fallback for unknown styles
        print(f"⚠️ Unknown subtitle style '{style}', using phrase-level fallback")
        events = generate_phrase_events(
            phrase_timings, clip_start, clip_end, style_config
        )

    print(
        f"  [DEBUG] ASS Generator: {len(events)} events generated for clip ({clip_start:.2f}s - {clip_end:.2f}s) | Style: {style}"
    )
